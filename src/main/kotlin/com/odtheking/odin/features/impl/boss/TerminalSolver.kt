...
object TerminalSolver : Module(
    ...
    val terminalReloadThreshold by NumberSetting("Resolve timeout", 600, 100, 2000, 10, unit = "ms", desc = "The amount of time before the terminal reloads after a click wasn't registered while using hide clicked.").withDependency { hideClicked && solverSettings }
    ...
    fun GuiGraphicsExtractor.renderDebug() {
        if (debug) TerminalUtils.currentTerm?.let { term ->
            ...
            textWithWordWrap(mc.font, Component.literal(items.filter { !it.isEmpty }.map { stack -> stack.hoverName.string }.toString()), 400, 0, 300, Colors.WHITE.rgba)
