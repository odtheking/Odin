...
object AutoIceFill: Module(
    ...
    private suspend fun move(pos: Vec3, pattern: List<Vec3i>, rotation: Rotation, floorIndex: Int) {
        ...
        val y = pos.yCoord
        val z = pos.zCoord
        val deferred1 = AsyncUtils.waitUntilPacked(x, y, z)
        try {
            deferred1.await()
        } catch (e: Exception) {
            return
        ...
        val deferred = AsyncUtils.waitUntilPacked(x + bx2, y, z + bz2)
        try {
            deferred.await()
        } catch (e: Exception) {
            return
    ...
    private fun clipToNext(pos: Vec3, rotation: Rotation, bx: Int, bz: Int, floorIndex: Int) {
        ...
        PlayerUtils.clipTo(x + bx + nx, y + 1.5, z + bz + ny)
        GlobalScope.launch {
            delay(100)
            PlayerUtils.clipTo(x + bx + nx * 2, y + 2, z + bz + ny * 2)
            delay(100)
