...
val dungeonWaypointsCommand = Commodore("dwp", "dungeonwaypoints") {
    ...
    literal("import").runs {
        scope.launch {
            val base64Data = GuiScreen.getClipboardString()?.replace("\n", "") ?: return@launch modMessage("Failed to read a string from clipboard. Did you copy it correctly?")
            if (base64Data.startsWith("{")) return@launch modMessage("It looks like you copied json data instead of base64. Ensure you copied the correct text!")
