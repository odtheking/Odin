...
object HudManager : Screen() {
    ...
    override fun draw() {
        dragging?.let {
            it.x = floor((odinMouseX + deltaX).coerceIn(0f, mc.displayWidth - (it.width * it.scale)))
            it.y = floor((odinMouseY + deltaY).coerceIn(0f, mc.displayHeight - (it.height * it.scale)))
        }
