...
object WardrobeKeybinds : Module(
    ...
    private fun onClick(gui: GuiChest, key: Int): Boolean {
        ...
        val index = when {
            nextPageKeybind.key == key -> if (current.toInt() < total.toInt()) 53 else return modMessage("§cYou are already on the last page.").let { false }
            previousPageKeybind.key == key -> if (current.toInt() > 1) 45 else return modMessage("§cYou are already on the first page.").let { false }
            unequipKeybind.key == key -> equippedIndex ?: return modMessage("§cCouldn't find equipped armor.").let { false }
            else -> {
                val keyIndex = arrayOf(wardrobe1, wardrobe2, wardrobe3, wardrobe4, wardrobe5, wardrobe6, wardrobe7, wardrobe8, wardrobe9).indexOfFirst { it.key == key }.takeIf { it != -1 } ?: return false
