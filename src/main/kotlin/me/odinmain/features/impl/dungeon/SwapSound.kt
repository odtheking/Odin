...
object SwapSound : Module(
    ...
    private val customSound by StringSetting("Custom Sound", "mob.blaze.hit", desc = "Name of a custom sound to play. This is used when Custom is selected in the Sound setting.").withDependency { sound == defaultSounds.size - 1 }
    ...
    private val reset by ActionSetting("Play sound", desc = "Plays the sound with the current settings.") { PlayerUtils.playLoudSound(if (sound == defaultSounds.size - 1) customSound else defaultSounds[sound], volume, pitch) }
