...
object BuildHelper : Module(
    ...
    fun renderWorldEvent(event: RenderWorldLastEvent) {
        ...
        if (stunNotificationNumber != 0 && KuudraUtils.buildDonePercentage >= stunNotificationNumber) PlayerUtils.alert("§lGo to stun", playSound = false, color = Colors.MINECRAFT_DARK_AQUA)
        if (buildHelperDraw)
        ...
        if (unfinishedWaypoints)
            KuudraUtils.buildingPiles.forEach {
                ...
                if (hideDefaultTag) it.alwaysRenderNameTag = false
            }
    }
