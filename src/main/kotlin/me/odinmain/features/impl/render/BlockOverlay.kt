...
object BlockOverlay : Module(
    ...
    private val style by SelectorSetting("Block Style", Renderer.DEFAULT_STYLE, Renderer.styles, description = Renderer.STYLE_DESCRIPTION).withDependency { blockOverlayToggle }
    private val color by ColorSetting("Block Color", Colors.BLACK.withAlpha(0.4f), allowAlpha = true, description = "The color of the box.").withDependency { blockOverlayToggle }
    private val lineWidth by NumberSetting("Block Line Width", 2f, 0.1f, 10f, 0.1f, description = "The width of the box's lines.").withDependency { blockOverlayToggle }
    ...
    private val entityToggle by BooleanSetting("Entity Hover", false, description = "Master toggle for Entity Hover feature.")
    ...
    private val entityColor by ColorSetting("Entity Color", Colors.WHITE.withAlpha(0.75f), true, description = "The color of the highlight.").withDependency { entityToggle }
    private val thickness by NumberSetting("Entity Line Width", 2f, 1f, 6f, .1f, description = "The line width of Outline / Boxes/ 2D Boxes.").withDependency { entityToggle && entityMode != HighlightRenderer.HighlightType.Overlay.ordinal }
    private val entityStyle by SelectorSetting("Entity Style", Renderer.DEFAULT_STYLE, Renderer.styles, description = Renderer.STYLE_DESCRIPTION).withDependency { entityToggle && entityMode == HighlightRenderer.HighlightType.Boxes.ordinal }
