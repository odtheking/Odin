...
class RenderEntityOutlineEvent(theType: Type?, potentialEntities: HashSet<Entity>?) : Event() {
    ...
    init {
        ...
        if (potentialEntities != null) entitiesToOutline = HashMap(potentialEntities.size)
    }
    ...
    fun queueEntitiesToOutline(outlineColor: ((entity: Entity) -> Int?)? = null) {
        ...
        if (entitiesToChooseFrom == null) computeAndCacheEntitiesToChooseFrom()
    ...
    private fun computeAndCacheEntitiesToChooseFrom() {
        val entities: List<Entity> = mc.theWorld.getLoadedEntityList()
