...
object MapInfo : Module(
    ...
    private val width: Float by NumberSetting("Width", default = 165f, min = 160f, max = 200f, increment = 1f).withDependency { alternate }
    ...
    val hud: HudElement by HudSetting("Hud", 10f, 10f, 1f, false) {
        ...
        val displayWidth = if (alternate) width else 160f
        ...
        if (background) roundedRectangle(0, 0, displayWidth-1, 19, color, 0, 0)
        val brWidth = getMCTextWidth(brText)
        val trWidth = getMCTextWidth(trText)
        mcText(secretText, 1, 1, 1f, Color.WHITE, center = false)
        mcText(trText, displayWidth-1 - trWidth, 1, 1f, Color.WHITE, center = false)
        val unknownWidth = mcTextAndWidth(unknownSecretsText, 1, 10, 1f, Color.WHITE, center = false)
        val centerX = (unknownWidth+1+(displayWidth-1-unknownWidth-brWidth)/2) - getMCTextWidth(mimicText)/2
        mcText(mimicText, centerX, 10, 1f, Color.WHITE, center = false)
        mcText(brText, displayWidth-1 - brWidth, 10, 1f, Color.WHITE, center = false)
        displayWidth to 19f
    }
