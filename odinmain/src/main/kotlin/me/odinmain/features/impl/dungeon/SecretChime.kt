...
object SecretChime : Module(
    ...
    private val sound by SelectorSetting("Sound", "mob.blaze.hit", defaultSounds, description = "Which sound to play when you get a secret.")
    ...
    val reset by ActionSetting("Play sound") { PlayerUtils.playLoudSound(if (sound == defaultSounds.size - 1) customSound else defaultSounds[sound], volume, pitch) }
    private val inBoss by BooleanSetting("In Boss Room", false, description = "Prevent playing the sound if in boss room.")
